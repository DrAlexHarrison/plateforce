...
plateforce compiled to WebAssembly, wrapped in a static site. The same compiled
`plateforce-core` runs here as in the CLI.

A trace is read by the browser's file API, handed to WebAssembly in the tab, and analysed
there. There is no server, no request carrying sample data, and no telemetry. Force traces
are re-identifiable, which is why the file is read where it already sits.
...
| `styles.css` | the tokens, then the components. No webfont is fetched, so the page makes no outbound requests |
...
written, so the desktop wrapper can load the same directory with no server assumptions and
no absolute paths.
