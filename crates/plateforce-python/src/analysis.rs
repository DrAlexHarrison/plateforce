...
struct AnalysisDocument<'a> {
    ...
    acquisition: plateforce_core::Acquisition,
    acquisition_complete: bool,
...
pub fn analyse_json(
    ...
    // Destructured rather than read field by field, so a fact the capture gains is a compile
    // error here rather than one this document quietly stops carrying.
    let plateforce_core::Capture {
        acquisition,
        plate_profile,
    } = trial.capture.clone().unwrap_or_default();
