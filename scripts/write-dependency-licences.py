...
def workflows_running_the_check() -> list[str]:
    """The workflows that run this script with --check, read rather than assumed.

    NOTICE tells a reader the tables are enforced, and a sentence about a check is worth
    nothing unless something asserts the check exists. Removing the last job that runs it
    makes that sentence false, which is the shape this file is here to refuse.
    """
    invocation = re.compile(r"write-dependency-licences\.py[^\n]*--check")
    return sorted(
        path.name
        for path in sorted(WORKFLOWS.glob("*.yml"))
        if invocation.search(path.read_text())
    )
...
def main() -> int:
    ...
    running = workflows_running_the_check()
    if not running:
        print(
            "NOTICE says CI runs this check and no workflow does, so the file tells a\n"
            "reader the tables are enforced when nothing enforces them.",
            file=sys.stderr,
        )
        return 1
    print(f"the check runs in {', '.join(running)}")
