/*
 * Vendored from the bundled `dataviz` skill, unmodified above the plateforce block at the
 * foot of this file.
 *
 * It is a copy rather than a reference because two documents in this project print a
 * command that runs it, and the rows those documents carry were measured by running it. A
 * measurement beside a command nobody can run is an assertion, and the path into a bundle
 * moves. `--self-check` re-measures those four rows, so they are a query here rather than
 * a table.
 */
...
// ── plateforce: the four rows two of this project's documents carry ────────────
//
// Each row was measured by running this validator, so the table is reproduced by running
// it again rather than by trusting the table. A validator tuned until it agrees with the
// rows it was meant to check is worth nothing, so nothing above this line was edited.

const MEASURED_ROWS = [
  { what: "3 event markers, light",
    palette: ["#b45309", "#b81d5b", "#1b5fa8"], mode: "light", surface: "#ffffff",
    expect: { "CVD separation": "PASS", "Normal-vision floor": "FAIL", "Lightness band": "PASS" },
    figures: ["9.6", "14.1"] },
  { what: "3 event markers, dark",
    palette: ["#e0a33c", "#f27ba6", "#6ea8e8"], mode: "dark", surface: "#12171c",
    expect: { "CVD separation": "PASS", "Normal-vision floor": "PASS", "Lightness band": "FAIL" },
    figures: ["10.3", "18.4", "0.756", "0.73", "0.718"] },
  { what: "3 event markers plus the takeoff-threshold line, light",
    palette: ["#b45309", "#b81d5b", "#1b5fa8", "#b42318"], mode: "light", surface: "#ffffff",
    expect: { "CVD separation": "FAIL", "Normal-vision floor": "FAIL" },
    figures: ["5.4", "8.4"] },
  { what: "3 event markers plus the takeoff-threshold line, dark",
    palette: ["#e0a33c", "#f27ba6", "#6ea8e8", "#f08b80"], mode: "dark", surface: "#12171c",
    expect: { "CVD separation": "FAIL", "Normal-vision floor": "FAIL" },
    figures: ["5.9", "7.4"] },
];
...
function selfCheck() {
  ...
  for (const row of MEASURED_ROWS) {
    const { report } = validate(row.palette, { mode: row.mode, surface: row.surface, pairs: "all" });
    const states = Object.fromEntries(report.map(([name, state]) => [name, GLYPH[state] ?? state]));
    const details = report.map(([, , detail]) => detail).join(" ");
    const wrong = Object.entries(row.expect).filter(([name, want]) => states[name] !== want);
    const missing = row.figures.filter((figure) => !details.includes(figure));
    if (wrong.length || missing.length) {
      failures += 1;
      console.log(`FAIL  ${row.what}`);
      for (const [name, want] of wrong) console.log(`        ${name} read ${states[name]}, the document records ${want}`);
      if (missing.length) console.log(`        figures the document records and this run did not print: ${missing.join(", ")}`);
      for (const [name, , detail] of report) console.log(`        ${name}: ${detail}`);
    } else {
      console.log(`pass  ${row.what}: ${Object.entries(row.expect).map(([n, s]) => `${n} ${s}`).join(", ")}, ${row.figures.join(" ")}`);
    }
  }
  console.log(`\n${MEASURED_ROWS.length - failures} of ${MEASURED_ROWS.length} recorded rows reproduce`);
  return failures === 0;
}
