...
use plateforce_analysis::{
    chains_of, descriptions_of, run, AnalysisRequest, AnalysisResponse, Binding, BINDINGS,
};
...
use plateforce_core::{Provenance, Sentinel, Trial};
...
struct AnalysisReport {
    ...
    /// `plateforce_analysis::descriptions_of` so a number reads the same in a terminal, a
    /// notebook, a browser tab and an R session. This boundary held that generator, so R was
    /// the only surface whose reader ever received one.
