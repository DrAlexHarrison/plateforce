...
use plateforce_analysis::{
    chains_of, run, AnalysisRequest, AnalysisResponse, Binding, MetricChain, BINDINGS,
};
...
use plateforce_core::{Measured, Provenance, Sentinel, Trial};
...
struct AnalysisReport {
    ...
    ///
    /// Derived by `plateforce_analysis::chains_of`, so the tree an R session reads is the tree
    /// a folder run and a notebook read. The R package used to build it from `bound_methods`
    /// and `contributing_method_ids` for itself, which put every contributing rule at one
    /// depth under a root that carried none of the arithmetic's own values.
...
fn run_and_report(handle: &TrialHandle, request: AnalyseRequest) -> String {
    ...
    match run(&handle.trial, &request.analysis) {
        Ok(response) => {
            let chains = chains_of(&response, &stamp, complete);
            let descriptions = descriptions_of(&response, &chains);
            ok(AnalysisReport {
                provenance: chains.into_iter().map(|one| one.chain.provenance).collect(),
                descriptions,
                response,
...
///
/// The chain each account is written around is `plateforce_analysis::chains_of`'s, walked
/// beside `metrics` because that call returns one entry per metric in the response's own
/// order. This used to build a tree of its own, which put every contributing rule at one depth
/// under a root carrying none of the arithmetic's own values, so the gravity behind the
/// flight-time height was absent from the sentence a reader was shown.
///
